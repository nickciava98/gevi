...
class ExportOperativo(models.TransientModel):
    ...
    def action_export_manutentori(self):
        if not self.programmazione_data_da or not self.programmazione_data_a:
            raise exceptions.ValidationError('ATTENZIONE: Le date devono essere valorizzate!')

        # Preparo le intestazioni per il foglio excel
        filename = 'ProssimeVerifiche_{0}_{1}.xls'.format(self.programmazione_data_da, self.programmazione_data_a)
        workbook = xlwt.Workbook(encoding="UTF-8")
        ws_name = "ProssimeVerifiche".format(self.programmazione_data_a, self.programmazione_data_a)
        ...
        request = ("SELECT CONCAT(gcm.name, ' ', gcm.provincia), " +\
                    ...
                    "WHERE data_programmazione >= '{0}' " +\
                        ...
                        "AND gvv.state in ('bozza','assegnato') ").format(self.programmazione_data_da, self.programmazione_data_a)
        if self.zona_impianto_ids:
            request = request + ("AND gzzi.id in {0} ").format(str(tuple(self.zona_impianto_ids.ids)).replace(',)',')'))
        ...
        for sql_row in self.env.cr.fetchall():
            ...
            worksheet.write(xls_row_index, 9, (datetime.strptime(sql_row[9], '%Y-%m-%d')).strftime('%d/%m/%Y') if sql_row[9] else "", style)
            worksheet.write(xls_row_index, 10, (datetime.strptime(sql_row[10], '%Y-%m-%d')).strftime('%d/%m/%Y')  if sql_row[10] else "", style)
            worksheet.write(xls_row_index, 11, (datetime.strptime(sql_row[11], '%Y-%m-%d')).strftime('%d/%m/%Y') if sql_row[11] else "", style)
    ...
    def action_export_ispettore(self):
        if not self.manutentore_ids or not self.zona_impianto_id:
            raise exceptions.ValidationError('ATTENZIONE: Tutti i campi devono essere valorizzati!')
        if self.template_export_isp == 'template1':
            self.export_assegnati_ispettore_v1()

    @api.one
    def export_assegnati_ispettore_v1(self):
        # Preparo le intestazioni per il foglio excel
        oggi = datetime.now().strftime("%Y-%m-%d")
        filename = 'Verifiche_{0}_{1}.xls'.format(self.zona_impianto_id.name, oggi)
        workbook = xlwt.Workbook(encoding="UTF-8")
        ws_name = "Verifiche_{0}_{1}.xls".format(self.zona_impianto_id.name, oggi)
        worksheet = workbook.add_sheet("Verifiche {0}".format(self.zona_impianto_id.name))
        # style = xlwt.easyxf('font:height 400, bold True, name Arial; align: horiz center, vert center;borders: top medium,right medium,bottom medium,left medium')
        style_date = xlwt.easyxf(num_format_str='DD-MM-YYYY')
        style_title = xlwt.easyxf('font:bold True')
        common_style = xlwt.easyxf('font:bold False')
        ...
        verbale_ids = self.env['gevi_verbali.verbale'].search([
            '&', '&',
            ('manutentore_id', 'in', self.manutentore_ids.ids),
            ('impianto_id.zona_impianto_id', '=', self.zona_impianto_id.id),
            ('state', '=', 'assegnato')
        ...
        for verbale in verbale_ids:
            ...
            matricola_obj = impianto_riga_desc_obj.search(['&', ('impianto_id', '=', verbale.impianto_id.id), ('name', '=', "Matricola")], limit=1)
            n_impianto_obj = impianto_riga_desc_obj.search(['&', ('impianto_id', '=', verbale.impianto_id.id), ('name', '=', "N. Impianto")], limit=1)
            marca_obj = impianto_riga_desc_obj.search(['&', ('impianto_id', '=', verbale.impianto_id.id), ('name', '=', "Marca")], limit=1)

            altro = ""
            if verbale.contratto_id.blocco_amministrativo:
                altro = altro + "{0}: {1} ".format("Blocco Ammm.", verbale.contratto_id.causale_blocco if verbale.contratto_id.causale_blocco else "")
                style = warning_style
            ...
            worksheet.write(xls_row_index, 4, verbale.impianto_id.indirizzo if verbale.impianto_id.indirizzo else "", style)
            worksheet.write(xls_row_index, 5, verbale.impianto_id.citta if verbale.impianto_id.citta else "", style)
            worksheet.write(xls_row_index, 6, matricola_obj.valore_attributo if matricola_obj.valore_attributo else "", style)
            worksheet.write(xls_row_index, 7, n_impianto_obj.valore_attributo if n_impianto_obj.valore_attributo else "", style)
            worksheet.write(xls_row_index, 8, marca_obj.valore_attributo if marca_obj.valore_attributo else "", style)
            worksheet.write(xls_row_index, 9, (datetime.strptime(verbale.data_programmazione, '%Y-%m-%d')).strftime('%d/%m/%Y') if verbale.data_programmazione else "", style)
            worksheet.write(xls_row_index, 10, (datetime.strptime(verbale.impianto_id.data_ultima_verifica, '%Y-%m-%d')).strftime('%d/%m/%Y') if verbale.impianto_id.data_ultima_verifica else "", style)
            worksheet.write(xls_row_index, 11, verbale.impianto_id.zona_impianto_id.name if verbale.impianto_id.zona_impianto_id else "", style)
    ...
    def action_export_confermati(self):
        if not self.conferma_data_da or not self.conferma_data_a:
            raise exceptions.ValidationError('ATTENZIONE: Le date devono essere valorizzate!')
        
        # Preparo le intestazioni per il foglio excel
        filename = 'VerificheConfermate_{0}_{1}.xls'.format(self.conferma_data_da, self.conferma_data_a)
        workbook = xlwt.Workbook(encoding="UTF-8")
        ws_name = "Verifiche Confermate dal {0} al {1}".format(self.conferma_data_da, self.conferma_data_a)
        ...
        verbale_ids = self.env['gevi_verbali.verbale'].search([
            '&', '&',
            ('data_conferma', '>=', self.conferma_data_da),
            ('data_conferma', '<=', self.conferma_data_a),
            ('state', '=', 'confermato')
        ...
        for verbale in verbale_ids:
            ...
            worksheet.write(xls_row_index, 2, (datetime.strptime(verbale.data_esecuzione, '%Y-%m-%d')).strftime('%d/%m/%Y'), style_date)
            worksheet.write(xls_row_index, 3, (datetime.strptime(verbale.data_programmazione, '%Y-%m-%d')).strftime('%d/%m/%Y') if verbale.data_programmazione else "", style_date)
            worksheet.write(xls_row_index, 4, (datetime.strptime(verbale.data_assegnazione, '%Y-%m-%d')).strftime('%d/%m/%Y'), style_date)
            worksheet.write(xls_row_index, 5, (datetime.strptime(verbale.data_conferma, '%Y-%m-%d')).strftime('%d/%m/%Y'), style_date)
            worksheet.write(xls_row_index, 6, verbale.name, style)
            worksheet.write(xls_row_index, 7, (datetime.strptime(verbale.data_verbale, '%Y-%m-%d')).strftime('%d/%m/%Y') if verbale.data_verbale else "", style_date)
            worksheet.write(xls_row_index, 8, verbale.manutentore_id.name_get()[0][1] if verbale.manutentore_id else "", style)
            worksheet.write(xls_row_index, 9, verbale.impianto_id.name_get()[0][1], style)
            worksheet.write(xls_row_index, 10, verbale.impianto_id.zona_impianto_id.name if verbale.impianto_id.zona_impianto_id else "", style)
    ...
    def action_export_eseguiti(self):
        if not self.esecuzione_data_da or not self.esecuzione_data_a:
            raise exceptions.ValidationError('ATTENZIONE: Le date devono essere valorizzate!')

        # Preparo le intestazioni per il foglio excel
        filename = 'VerificheEseguite_{0}_{1}.xls'.format(self.esecuzione_data_da, self.esecuzione_data_a)
        workbook = xlwt.Workbook(encoding="UTF-8")
        ws_name = "Verifiche Eseguite dal {0} al {1}".format(self.esecuzione_data_da, self.esecuzione_data_a)
        ...
        verbale_ids = self.env['gevi_verbali.verbale'].search([
            '&', '&',
            ('data_esecuzione', '>=', self.esecuzione_data_da),
            ('data_esecuzione', '<=', self.esecuzione_data_a),
            ('state', '=', 'eseguito')
        ...
        for verbale in verbale_ids:
            ...
            worksheet.write(xls_row_index, 3, (datetime.strptime(verbale.data_programmazione, '%Y-%m-%d')).strftime('%d/%m/%Y') if verbale.data_programmazione else "", style_date)
            worksheet.write(xls_row_index, 4, (datetime.strptime(verbale.data_assegnazione, '%Y-%m-%d')).strftime('%d/%m/%Y'), style_date)
            worksheet.write(xls_row_index, 5, verbale.manutentore_id.name_get()[0][1] if verbale.manutentore_id else "", style)
            worksheet.write(xls_row_index, 6, verbale.impianto_id.name_get()[0][1], style)
            worksheet.write(xls_row_index, 7, verbale.impianto_id.zona_impianto_id.name if verbale.impianto_id.zona_impianto_id else "", style)
    ...
    def action_export_ministero(self):
        if not self.verbale_data_da or not self.verbale_data_a:
            raise exceptions.ValidationError('ATTENZIONE: Le date devono essere valorizzate!')

        # Preparo le intestazioni per il foglio excel
        filename = 'Ministero_{0}_{1}.xls'.format(self.verbale_data_da, self.verbale_data_a)
        workbook = xlwt.Workbook(encoding="UTF-8")
        ws_name = "Elenco Certificati dal {0} al {1}".format(self.verbale_data_da, self.verbale_data_a)
        ...
        request = ("SELECT CASE WHEN giic.name LIKE 'Asc%' THEN 'Ascensore' WHEN giic.name LIKE 'Pia%' THEN 'Piattaforma Elevatrice' WHEN giic.name LIKE 'Montac%' THEN 'Montacarichi' WHEN giic.name LIKE 'Montas%' THEN 'Montascale' WHEN giic.name LIKE 'Mess%' THEN 'Messa a Terra' WHEN giic.name LIKE 'Scar%' THEN 'Scariche Atmosferiche' END AS TipoImpianto, CASE WHEN gvv.periodica is True THEN 'Periodica' WHEN gvv.periodica is False THEN 'Straordinaria' END AS TipoVerbale, CASE WHEN gii.proprietario_diverso is True THEN rppd.name ELSE rpcf.name END AS Cliente, gii.indirizzo AS UbicazioneImpianto, gii.citta AS Localita, gii.provincia AS Prov, gvv.data_verbale AS DataCertificato, gvv.name AS NrVerbale, CASE WHEN giird.name = 'Matricola' THEN giird.valore_attributo WHEN giird.name = 'Potenza' THEN giird.valore_attributo WHEN giird.name = 'Tipo di captatore%' THEN giird.valore_attributo ELSE '-' END AS Matricola, gvv.esito_verifica AS Esito, hre.name_related AS Verificatore, hre.identification_id AS CodiceFiscaleVerificatore, gvv.state AS Stato " +\
                   ...
                   "ORDER BY gvv.name").format(self.verbale_data_da, self.verbale_data_a)
        #_logger.info('******************************** QueryMinistero: {0}'.format(request))
        title = worksheet.row(0)
        ...
        for sql_row in self.env.cr.fetchall():
            ...
            worksheet.write(xls_row_index, 4, sql_row[4], style)
            worksheet.write(xls_row_index, 5, sql_row[5], style)
            worksheet.write(xls_row_index, 6, (datetime.strptime(sql_row[6], '%Y-%m-%d')).strftime('%d/%m/%Y'), style)
            worksheet.write(xls_row_index, 7, sql_row[7], style)
            worksheet.write(xls_row_index, 8, sql_row[8], style)
