        ...
        for line in self:
            ...
            request = ("SELECT CONCAT(gcm.name, ' ', gcm.provincia), " +\
                        ...
                        "WHERE data_programmazione >= '{0}' " +\
                            ...
                            "AND gvv.state in ('bozza','assegnato') ").format(line.programmazione_data_da, line.programmazione_data_a)
            if line.zona_impianto_ids:
                request = request + ("AND gzzi.id in {0} ").format(str(tuple(line.zona_impianto_ids.ids)).replace(',)',')'))
