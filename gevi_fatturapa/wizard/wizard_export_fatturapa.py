...
class WizardExportFatturapa(models.TransientModel):
    ...
    report_print_menu = fields.Many2one(
        ...
        help="Questo report verrà automaticamente incluso nell XML creato")
    ...
    def saveAttachment(self, fatturapa, number):
        ...
        if not company.vat:
            raise UserError(
                _('La partita iva di %s non è impostata.') % company.name)
        if (
    ...
    def setProgressivoInvio(self, fatturapa):
        ...
        # La richiesta del cliente è quella di avere progressivo uguale a numero fattura
        ...
        if not fatturapa_sequence:
            raise UserError("La sequence per la Fattura Elettronica non è configurata")
    ...
    def _setIdTrasmittente(self, company, fatturapa):
        ...
        if not company.country_id:
            raise UserError(
                "Il codice nazione dell'azienda non è impostato")
        ...
        if not IdCodice:
            raise UserError("La partita iva dell'azienda non è impostata")
    ...
    def _setCodiceDestinatario(self, partner, fatturapa):
        ...
        if partner.is_pa():
            if not partner.codice_ipa:
                raise UserError(
                    "Il cliente {} è una Pubblica Amministrazione ma il codice IPA non è impostato".format(partner.name))
            code = partner.codice_ipa
        else:
            if not partner.codice_destinatario:
                raise UserError(
                    "Il cliente {} non è una Pubblica Amministrazione ma il Codice Destinatario non è impostato".format(partner.name))
    ...
    def _setContattiTrasmittente(self, company, fatturapa):
        ...
        if not company.phone:
            raise UserError("Il numero di telefono dell'azienda non è impostato")
        Telefono = company.phone

        if not company.email:
            raise UserError("L'indirizzo email dell'azienda non è impostato")
        Email = company.email
        fatturapa.FatturaElettronicaHeader.DatiTrasmissione. \
    ...
    def _setDatiAnagraficiCedente(self, CedentePrestatore, company):
        ...
        if not fatturapa_fp:
            raise UserError(
                ...
                "non è impostata per l'azienda {}. "
                "(Impostare su Contabilità --> Configurazione --> Impostazioni --> "
    ...
    def _setSedeCedente(self, CedentePrestatore, company):
        ...
        if not company.city:
            raise UserError("Impostare la città dell'azienda")
    ...
    def _setSedeCessionario(self, partner, fatturapa):
        ...
        if not partner.city:
            raise UserError("Impostare la città del Cliente")
